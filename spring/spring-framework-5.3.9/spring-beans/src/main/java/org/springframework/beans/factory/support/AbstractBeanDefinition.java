...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * bean标签的autowire属性为no
	 * 1、在xml中需要手动指定依赖注入对象，配置property标签或者constructor-arg标签
	 * 2、使用@Autowired注解，autowireMode的值也是0
	 * <p>
	...
	/**
	 * autowireMode = 1：根据set方法的名称作为Bean名称进行依赖查找（去掉set，并将首字母变为小写，
	 * 然后将对象设置到该set方法的参数上
	 * bean标签的autowire属性配置为byName
	 * <p>
	...
	/**
	 * autowireMode = 2：根据set方法的参数类型作为Bean类型进行依赖查找，并将对象设置到该set方法的参数上
	 * bean标签的autowire属性值配置为byType
	 * <p>
	 ...
	 * bean标签的autowire属性配置为constructor
	 * <p>
	...
	/**
	 * 表名通过Bean的class的内部来自动装配，Spring3.0以后弃用
	 * bean标签的autowire属性值配置为autodetect
	 * <p>
	...
	/**
	 * 若Bean为指定小辉方法，容器应该尝试推断Bean的销毁方法的名字
	 * 目前，推断的销毁方法的名字一般为close或shutdown
	 * <p>
	...
	/**
	 * 自动装配出现多个候选Bean时，将作为首选者，对应Bean属性Primary，默认不是首选的
	 *
	 ...
	 * 如果容器中有多个相同类型的bean，可以使用qualifier属性设置加载指定Bean名称的bean
	 *
	...
	/**
	 * 是否执行initMethod，默认true
	 */
	private boolean enforceInitMethod = true;

	/**
	 * 是否执行destroyMethod，默认true
	 */
	private boolean enforceDestroyMethod = true;

	/**
	 * 是否是用户定义而不应用程序定义的，创建AOP时为true
	 */
